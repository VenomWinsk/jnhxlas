...
class ReEngine(object):
    ...
    def replacehxGrok(self, role, srcStr):
        '''
        ...
        if role["fileds"]:
            role["fileds"].append(b)
        else:
            role["fileds"] = [b]

        return "(" + a + ")"

    def replaceRole(self, role, srcStr):
        '''
        替换整条规则
        :param srcStr:
        :param roleName:
        ...
        dstStr = re.subn(r'%\{.*?\}', lambda x: self.replacehxGrok(role, x.group()), srcStr)
        return dstStr[0]
