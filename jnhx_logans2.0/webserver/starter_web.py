...
startcmd_pylog = "/usr/bin/python3 /opt/hxht/hx_logprocess/webserver/webserver.py >> " \
                 "/data/data0/log/hx_logimporter.log & "


def read_flag():
    try:
        with open("/opt/hxht/hx_logprocess/webserverdata/run.flag", 'r') as reader:
            s = reader.read()
            return s
    except Exception:
        return "0"
...
def stop():
    programs = os.popen("ps -fe | grep 'webserver' | grep -v 'grep' | awk '{print $2}'").read().strip()
    ss = programs.split('\n')
    for pid in ss:
        os.popen("kill -9 " + pid)
        print("关闭程序" + pid)
